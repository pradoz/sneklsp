...
pub fn semantic_diagnostics(
    ...
    scope: Option<sneklsp_text::TextRange>,
) -> Vec<Diagnostic> {
...
impl<'a> DiagnosticCollector<'a> {
    ...
    fn run(&mut self, scope: Option<sneklsp_text::TextRange>) {
        self.check_references(scope);
        self.check_symbols(scope);
        if scope.is_none() {
            self.check_scopes();
        }
    }

    fn check_symbols(&mut self, scope: Option<sneklsp_text::TextRange>) {
        ...
        for symbol in self.index.symbols() {
            ...
            if let Some(range) = scope {
                if !symbol.range.overlaps(range) {
                    continue;
                }
            }
    ...
    fn check_references(&mut self, scope: Option<sneklsp_text::TextRange>) {
        for reference in self.index.references() {
            if let Some(range) = scope {
                if !reference.range.overlaps(range) {
                    continue;
                }
            }
